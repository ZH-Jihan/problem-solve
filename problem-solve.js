...
function sortCarsByYear(carsList) {
...
function findAndModifyPerson(arr, name, newAge) {
    ...
    const result = arr.map((person) => {
        if (person.name === name) {
            return Object.assign(Object.assign({}, person), { age: newAge }); // নতুন age সহ নতুন object তৈরি করা
        }
...
const numbersWithDuplicates = [1, 1, 2, 3, 4, 4, 5, 6, 7, 7, 8];
...
const calculateStudentGrades = (grades) => {
    const total = grades.reduce((acc, curr) => acc + curr, 0);
    const avg = total / grades.length;
    return avg;
};
const sortStudentsByAverageGrade = (studentsList) => {
...
console.log("Sorted Students by Average Grade:", sortStudentsByAverageGrade(students));
const items = [
    { quantity: 2, price: 10 },
    { quantity: 5, price: 4 },
    { quantity: 1, price: 20 },
